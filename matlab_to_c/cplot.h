...
void CPlot::DrawAxis(IplImage *image) {
	...
	for (int i = 0; i < ceil((x_axis_pos - bs) / y_scale_pixes) + 1; i++) {
		_snprintf_s(text, sizeof(text) - 1, "%.3f", i * y_scale_pixes / this->y_scale + this->y_min);                      // add by zhyh2010
	...
	for (int i = 1; i < ceil((h - x_axis_pos - bs) / y_scale_pixes) + 1; i++) {
		_snprintf_s(text, sizeof(text) - 1, "%.3f", -i * y_scale_pixes / this->y_scale + this->y_min);
		cvPutText(image, text, cvPoint(bs / 5, x_axis_pos + i * y_scale_pixes), &font, this->axis_color);
	}

	// x_max
	_snprintf_s(text, sizeof(text) - 1, "%.3f", this->x_max);
	cvPutText(image, text, cvPoint(w - bs / 2 - strlen(text) * chw, x_axis_pos), &font, this->text_color);

	// x min
	_snprintf_s(text, sizeof(text) - 1, "%.3f", this->x_min);
	...
	for (int i = 1; i < ceil(gw / x_scale_pixes) + 1; i++) {
		_snprintf_s(text, sizeof(text) - 1, "%.0f", this->x_min + i * x_scale_pixes / this->x_scale);
...
public:
	...
	Mat figure()
	{
		...
		Mat mat = cvarrToMat(this->Figure);
		return mat;
	}
};
